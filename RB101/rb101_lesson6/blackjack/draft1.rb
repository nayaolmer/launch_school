...
(4) Give dealer card
(5) Give player card
(6) Reveal dealer card 
(7) Player decides to hit until stay
(8) Dealer reveals cards
(9) Dealer hits until 17 points is reached
(10) Scores are calculated
...
loop do
  ...
  loop do 
    player_action_taken = player_hit_or_stay('Player', player_hand, dealer_hand)
    prompt("Action taken: #{player_action_taken}")
    break if ['Stay', 'Bust'].include?(player_action_taken)
    player_hand << deal_card!(playing_deck)
  ...
  loop do 
    ...
    break if ['Stay', 'Bust'].include?(dealer_action_taken)
    dealer_hand << deal_card!(playing_deck)
